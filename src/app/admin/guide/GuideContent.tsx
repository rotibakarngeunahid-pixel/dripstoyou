    ...
    },
    {
      ...
      blocks: [
        ...
        },
        {
          type: 'steps',
          items: id ? [
            { title: 'Kelola Kategori', desc: 'Klik "Kelola Kategori" di kanan atas. Di sini kategori item BUKAN daftar tetap — Anda bisa "+ Tambah Kategori" (nama bebas, misal "Suplemen"), edit nama/urutan lewat ikon pensil, atau nonaktifkan lewat toggle Status supaya tidak muncul lagi saat menambah item baru (item lama yang masih memakainya tidak terpengaruh).', detail: 'Hapus kategori: kalau masih dipakai N item, sistem minta Anda pilih kategori pengganti dulu — semua item itu otomatis dipindahkan sebelum kategori lama benar-benar dihapus. Tidak bisa menghapus kategori terakhir yang tersisa.' },
            { title: 'Tambah item baru', desc: 'Klik "+ Tambah Item" di kanan atas. Isi nama, kategori (dari daftar yang sudah Anda kelola), satuan, stok awal, stok minimum, tanggal expired, supplier, dan harga per unit.', detail: 'Stok minimum menentukan kapan item ditandai "Menipis" — sesuaikan dengan kecepatan pemakaian tiap item.' },
            ...
            { title: 'Manage Categories', desc: 'Click "Kelola Kategori" (Manage Categories) top right. Categories are NOT a fixed list — you can "+ Tambah Kategori" (any name, e.g. "Supplements"), rename/reorder via the pencil icon, or deactivate via the Status toggle so it no longer appears when adding new items (existing items keeping it are unaffected).', detail: 'Deleting a category: if it\'s still used by N items, the system asks you to pick a replacement category first — those items are automatically moved before the old category is actually deleted. You cannot delete the last remaining category.' },
            { title: 'Add a new item', desc: 'Click "+ Tambah Item" top right. Fill in name, category (from the list you manage), unit, starting stock, minimum stock, expiry date, supplier, and price per unit.', detail: 'Minimum stock determines when an item is flagged "Low" — tune it to each item\'s usage rate.' },
