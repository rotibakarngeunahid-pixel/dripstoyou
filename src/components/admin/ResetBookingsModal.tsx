...
export const RESET_CONFIRMATION_PHRASE = 'HAPUS SEMUA TRANSAKSI';
...
export function ResetBookingsModal({
  open, count, loading, error, reason, confirmText,
  ...
  const canConfirm = reason.trim().length >= 5 && confirmText === RESET_CONFIRMATION_PHRASE && !loading;
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          <span style={{ color: '#888' }}>{t.resetTransaksiCountLabel}:</span>{' '}
          <span style={{ fontWeight: 800, color: 'var(--teal)' }}>{count}</span>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          ...
          >
            {loading ? t.sedangMereset : t.yaResetSemua}
          </button>
        </div>
      </div>
    </div>
